...
async function hasColumn(db: Database, table: string, column: string): Promise<boolean> {
  const cols = await db.select<any[]>(`PRAGMA table_info(${table})`);
...
async function migrate(db: Database) {
  ...
  await ensureColumn(db, "meal_plan", "notes", "TEXT DEFAULT ''");
  await ensureColumn(db, "meal_plan", "include_in_shopping_list", "INTEGER DEFAULT 1");
  await ensureColumn(db, "meal_plan", "sort_order", "INTEGER DEFAULT 0");
  await ensureColumn(db, "meal_plan", "created_at", "TEXT");

  await db.execute(`
    UPDATE meal_plan
    SET notes = COALESCE(notes, ''),
        include_in_shopping_list = COALESCE(include_in_shopping_list, 1),
...
export async function saveRecipe(recipe: Partial<Recipe> & { id?: string }): Promise<Recipe> {
  ...
  return {
    ...
    is_vegetarian: !!saved.is_vegetarian,
    is_vegan: !!saved.is_vegan,
    is_gluten_free: !!saved.is_gluten_free,
    is_dairy_free: !!saved.is_dairy_free,
    is_spicy: !!saved.is_spicy,
    is_high_protein: !!saved.is_high_protein,
...
export async function loadMealPlan(weekStart: string): Promise<MealPlanEntry[]> {
  ...
  return rows.map((row) => ({
    ...
    include_in_shopping_list:
      row.include_in_shopping_list === null || row.include_in_shopping_list === undefined
        ? true
        : !!row.include_in_shopping_list,
...
export async function addMealPlanItem(
  ...
  const rows = await db.select<{ next_sort: number }[]>(
    `
  ...
  const nextSort = rows[0]?.next_sort ?? 0;

  await db.execute(
    `
      INSERT INTO meal_plan (
        id, week_start, day_of_week, meal_slot, recipe_id,
        servings, notes, include_in_shopping_list, sort_order
      ) VALUES (?,?,?,?,?,?,?,?,?)
    `,
    [uuid(), weekStart, dayOfWeek, slot, recipeId, servings, "", 1, nextSort]
  );
}
...
export async function updateMealPlanItem(
  ...
  if ("recipe_id" in patch) {
    sets.push("recipe_id = ?");
    params.push(patch.recipe_id ?? null);
  }

  if ("servings" in patch) {
    sets.push("servings = ?");
    params.push(patch.servings ?? 1);
  }
  ...
  if ("include_in_shopping_list" in patch) {
    sets.push("include_in_shopping_list = ?");
    params.push(patch.include_in_shopping_list ? 1 : 0);
  }

  if ("sort_order" in patch) {
    sets.push("sort_order = ?");
    params.push(patch.sort_order ?? 0);
  }
  ...
  await db.execute(
    `UPDATE meal_plan SET ${sets.join(", ")} WHERE id = ?`,
    params
  );
}
...
export async function removeMealPlanItem(id: string): Promise<void> {
  const db = await getDb();
  await db.execute("DELETE FROM meal_plan WHERE id = ?", [id]);
}

// Legacy helper so older UI code does not instantly break
export async function setMealSlot(
  ...
  if (recipeId === null) {
    if (existing.length > 0) {
      await db.execute("DELETE FROM meal_plan WHERE id = ?", [existing[0].id]);
    }
    return;
  }
