...
const VOL: Record<string, number> = {
  ml: 1, l: 1000, L: 1000, 'fl oz': 29.57,
  cup: 236.6, cups: 236.6, tbsp: 14.79, tsp: 4.93,
};
...
export function scaleAmount(ing: Ingredient, ratio: number): string {
  ...
  if (ut === 'vol') {
    ...
    if (ml >= 900) return `${(ml / 1000).toFixed(1)}l`;
  ...
  for (const [n, d, sym] of fracs) {
    if (Math.abs(frac - n / d) < 0.08) return whole ? `${whole} ${sym}` : sym;
  }
...
export function aggregateIngredients(
  ...
  for (const { ingredients, baseServings, scaledServings } of pairs) {
    ...
    for (const ing of ingredients) {
      ...
      const totalBase =
        ut === 'mass' ? baseAmt * (MASS[ing.unit] ?? 1) :
        ut === 'vol'  ? baseAmt * (VOL[ing.unit] ?? 1)  :
        baseAmt;

      if (!agg[key]) {
        agg[key] = { name: ing.name.replace(/,.*$/, '').trim(), total: 0, ut, unit: ing.unit };
      }
  ...
  return Object.values(agg).map((item) => {
    let display = '';
    if (item.ut === 'mass') {
      display = item.total >= 900
        ? `${(item.total / 1000).toFixed(2).replace(/\.?0+$/, '')}kg`
        : `${Math.round(item.total)}g`;
    } else if (item.ut === 'vol') {
      const ml = item.total;
      display = ml >= 900 ? `${(ml / 1000).toFixed(1)}l`
        : ml >= 28 ? `${Math.round(ml / 14.79)} tbsp`
...
const PALETTE_COLORS = [
  '#C26A45', '#6B7A3E', '#3A6A9A', '#8B5E9A',
  '#2E7D5E', '#B07030', '#A03060', '#4A8080',
];
