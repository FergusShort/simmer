...
import {
  Recipe, Collection, FilterState, MealPlanEntry, CookLogEntry, SortOption
} from '@/types';
import {
  loadAllRecipes, saveRecipe, deleteRecipe, toggleFavorite, toggleArchive,
  addCookLog, loadCollections, createCollection, setRecipeCollection,
  loadMealPlan, setMealSlot, parsedToRecipe, pickBgColor,
} from '@/lib/db';
...
const DEFAULT_FILTERS: FilterState = {
  search: '', isVegetarian: false, isVegan: false, isGlutenFree: false,
  isDairyFree: false, isHighProtein: false, isMealPrep: false, isSpicy: false,
  isFavouritesOnly: false, isQuick: false, showArchived: false,
  maxTime: 100, minRating: 0, cuisines: [], tags: [], sort: 'newest',
};
...
// Derived selector - filtered + sorted recipes
export function useFilteredRecipes() {
  ...
  if (filters.search.trim()) {
    ...
    base = base.filter((r) => {
      const hay = [
        r.name, r.cuisine, r.description, r.notes,
  ...
  // Rating
  if (filters.minRating > 0) base = base.filter((r) => (r.rating || 0) >= filters.minRating);

  // Cuisines
  if (filters.cuisines.length > 0) base = base.filter((r) => filters.cuisines.includes(r.cuisine));
  ...
  // Sort
  switch (filters.sort) {
    case 'name': base.sort((a, b) => a.name.localeCompare(b.name)); break;
    case 'rated': base.sort((a, b) => (b.rating || 0) - (a.rating || 0)); break;
    case 'time': base.sort((a, b) => a.total_time - b.total_time); break;
    case 'cooked':
      base.sort((a, b) => {
      ...
      break;
    default: base.sort((a, b) => b.date_added.localeCompare(a.date_added));
  }
